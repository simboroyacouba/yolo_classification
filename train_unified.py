"""
Entrainement YOLO unifie - toutes classes, un seul modele

Entraine un unique modele YOLO sur l'ensemble du dataset (toutes images,
toutes classes) sans separation nadir / oblique.

Classes : panneau_solaire, batiment_peint, batiment_non_enduit,
          batiment_enduit, menuiserie_metallique

Usage :
  python train_unified.py
  python train_unified.py --freeze-epochs 5
  python train_unified.py --attention cbam
  python train_unified.py --images-dir ../dataset1/images/default
"""
...
def load_classes(yaml_path):
    if not os.path.exists(yaml_path):
        raise FileNotFoundError(
            ...
            f"Conseil : verifiez le chemin ou lancez d'abord split_dataset.py"
        )
...
def oversample_train_set(images_dir, labels_dir, classes, weights_map):
    """
    Duplique les images d'entrainement pour les classes sous-representees.
    weights_map : {class_name: facteur}  — facteur 1 = pas de copie.
...
def prepare_yolo_dataset(images_dir, annotations_file, output_dir, classes,
    ...
    print(f"   Classes : {[coco.cats[c]['name'] for c in valid_cat_ids]}")
    ...
    active_weights = {k: v for k, v in OVERSAMPLE_WEIGHTS.items() if v > 1}
    if active_weights:
        print("\n   Oversampling :")
        for cls_name, w in OVERSAMPLE_WEIGHTS.items():
            print(f"      {cls_name:<30} x{w}")
        n_copies = oversample_train_set(
            dirs["train_img"], dirs["train_lbl"],
            classes, OVERSAMPLE_WEIGHTS,
        )
    ...
    train_imgs_os = len(list(Path(dirs["train_img"]).glob("*.*"))) if has_os else stats["train"]
    ...
    if has_os:
        ...
        print(f"   {'Val':<10} {stats['val']:>8}  {stats['val']:>9}  {stats['ann_per_split']['val']:>12}")
        print(f"   {'Test':<10} {stats['test']:>8}  {stats['test']:>9}  {stats['ann_per_split']['test']:>12}")
        print(f"   {'-'*45}")
        print(f"   {'Total':<10} {total_imgs:>8}  {'-':>9}  {stats['annotations']:>12}")
    else:
        print(f"   {'Split':<10} {'Images':>8}  {'Annotations':>12}")
        print(f"   {'-'*34}")
        print(f"   {'Train':<10} {stats['train']:>8}  {stats['ann_per_split']['train']:>12}")
        print(f"   {'Val':<10} {stats['val']:>8}  {stats['ann_per_split']['val']:>12}")
        print(f"   {'Test':<10} {stats['test']:>8}  {stats['ann_per_split']['test']:>12}")
        print(f"   {'-'*34}")
        print(f"   {'Total':<10} {total_imgs:>8}  {stats['annotations']:>12}")
...
def make_per_class_ap_callback():
    def on_fit_epoch_end(trainer):
        ...
        if ap_class_index is None or ap_matrix is None or len(ap_class_index) == 0:
            return
...
def plot_training_curves(history, train_dir):
    if not history.get("mAP50"):
        return

    epochs = range(1, len(history["mAP50"]) + 1)
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    fig.suptitle("Courbes d'entrainement YOLO — Modele Unifie", fontsize=13)
...
def train_yolo(config):
    ...
    print("=" * 70)
    print(f"   YOLO ({config['model_version']}{config['model_size']}) — Modele UNIFIE (toutes classes)")
    print("=" * 70)
    print(f"\nConfiguration :")
    ...
    with open(os.path.join(train_dir, "training_report.txt"), "w", encoding="utf-8") as f:
        f.write(f"YOLO ({config['model_version']}{config['model_size']}) — Modele Unifie\n")
        f.write("=" * 50 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Entrainement YOLO unifie — un seul modele, toutes classes",
    ...
    parser.add_argument(
        ...
        choices=["none", "cbam"],
        default="none",
        help="Mecanisme d'attention (default: none).",
    )
    ...
    if not config["classes"]:
        print("Erreur : aucune classe trouvee dans le fichier classes YAML")
        return
